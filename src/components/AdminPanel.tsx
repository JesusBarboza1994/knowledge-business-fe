...
export function AdminPanel({ currentUserId, members, areas, onMemberSaved, onAreaSaved }: Props) {
  ...
  return (
    <div className="admin-panel flex min-h-0 flex-1 flex-col overflow-auto bg-ink">
      <header className="admin-header flex min-h-[62px] items-center gap-3 border-b border-line px-6">
        <div className={`grid h-9 w-9 place-items-center rounded-lg ${section === 'people' ? 'bg-accent/10 text-accent' : 'bg-accent/10 text-accent'}`}>
          {section === 'people' ? <Users size={18} /> : <FolderKanban size={18} />}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-2xs uppercase tracking-label text-muted">Administración</p>
        ...
        <div className="divide-y divide-line">
          {members.map((member) => (
            <button key={member.id} onClick={() => onSelect(member.id)} className={`member-row ${selected?.id === member.id ? 'active' : ''}`}>
              ...
              {member.role !== 'member' && <Shield size={14} className={member.role === 'superadmin' ? 'text-warn' : 'text-accent'} />}
            </button>
          ))}
        </div>
      </section>
          ...
          <>
            <div className="border-b border-line p-5">
              <div className="member-detail-head flex items-start gap-4">
                ...
                <span className="rounded-full border border-line px-2.5 py-1 text-2xs uppercase tracking-label text-muted">{roleLabels[selected.role]}</span>
              </div>
            </div>
            ...
            <div className="border-b border-line p-5">
              <div className="mb-4 flex items-center justify-between">
                ...
                {saved && <span className="flex items-center gap-1 text-xs text-accent"><Check size={13} /> Guardado</span>}
              </div>
              {error && <p className="mb-4 rounded-lg border border-danger/25 bg-danger/10 px-3 py-2 text-xs text-danger">{error}</p>}
              {protectsIdentity && <p className="mb-4 rounded-lg border border-warn/25 bg-warn/10 px-3 py-2 text-xs leading-5 text-warn">{selected.role === 'superadmin' ? 'El rol y estado de un superadministrador no se modifican desde esta consola.' : 'Para evitar perder acceso, no puedes cambiar tu propio rol ni desactivar tu cuenta.'}</p>}
            ...
            <div className="p-5">
              ...
              <div className="space-y-2">
                {areas.map((area) => (
                  <div key={area.key} className="admin-area-row flex items-center gap-3 rounded-lg border border-line bg-sunken p-3">
                    ...
                    <span className="relative">
                      <select value={selected.memberships[area.key] ?? 'none'} disabled={selected.role === 'superadmin' || saving} onChange={(event) => updateMembership(area.key, event.target.value as AccessLevel | 'none')} className="admin-access-select appearance-none rounded-md border-line bg-panel py-1.5 pl-3 pr-8 text-xs text-fg focus:border-accent/40 focus:ring-accent/20 disabled:opacity-50"><option value="none">Sin acceso</option><option value="read">Lectura</option><option value="write">Escritura</option><option value="manage">Administración</option></select>
        ...
        <div className="divide-y divide-line">
          {areas.map((area) => (
            <button key={area.key} onClick={() => onSelect(area.key)} className={`member-row ${selected?.key === area.key ? 'active' : ''}`}>
              <span className="grid h-9 w-9 flex-none place-items-center rounded-lg border border-line bg-sunken"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: area.color }} /></span>
  ...
  return (
    <form onSubmit={submit}>
      <div className="flex items-start gap-4 border-b border-line p-5">
        <span className="grid h-11 w-11 flex-none place-items-center rounded-xl border border-line bg-sunken"><i className="h-3 w-3 rounded-full" style={{ backgroundColor: area.color }} /></span>
