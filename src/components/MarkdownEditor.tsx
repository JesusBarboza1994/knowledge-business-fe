...
export function MarkdownEditor({
  ...
  const [historyVersions, setHistoryVersions] = useState<NoteVersion[] | null>(
    null,
  );
  ...
  useEffect(() => {
    setTitle(note.title);
    setBody(note.body);
    setSensitivity(note.sensitivity);
    setVisibleTo(note.visibleTo);
  ...
  const extensions = useMemo(() => {
    const wikiCompletion = (context: CompletionContext) => {
      ...
      const followingText = context.state.sliceDoc(
        context.pos,
        context.pos + 2,
      );
      return {
  ...
  async function save(forceVersion?: number) {
    ...
    try {
      const saved = await api.saveNote(
        note.id,
        { title, body, sensitivity, visibleTo },
        forceVersion ?? baseVersion.current,
      );
  ...
  async function openHistory() {
    setHistoryOpen(true);
    try {
      setHistoryVersions(await api.getVersions(note.slug));
    } catch {
      setHistoryVersions(note.versions);
    }
  }
  ...
  const status = {
    ...
    saving: (
      <>
        <span className="loader small" /> Guardando
      </>
    ),
    error: (
      <>
        <TriangleAlert size={13} /> Revisa el conflicto
      </>
    ),
  }[saveState];
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-ink">
      <header className="editor-header flex min-h-[62px] items-center gap-3 border-b border-line px-5">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2 text-2xs uppercase tracking-label text-muted">
            {note.kind !== "note" && (
        ...
        <div className="hidden rounded-lg border border-line bg-sunken p-1 sm:flex">
          <button
        ...
        {mode !== "edit" && (
          <article className="markdown-body overflow-auto px-7 py-8 lg:px-10">
            <ReactMarkdown
              ...
              components={{
                img: ({ src, alt, ...props }) => {
                  const assetId = assetIdFromRef(
                    typeof src === "string" ? src : undefined,
                  );
                  return (
                    <img
                      {...props}
                      src={assetId ? api.assetUrl(assetId) : src}
                      alt={alt ?? ""}
                      loading="lazy"
                      className="kb-asset"
                    />
                        ...
                        title={
                          target
                            ...
                            : link?.restricted
                              ...
                              : "La nota referenciada todavía no existe"
                        }
                      >
                        {children}
                      </button>
      ...
      {canEdit && (
        <footer className="editor-settings-footer flex min-h-11 items-center gap-3 border-t border-line px-5 text-xs text-muted">
          <button
            className="mobile-editor-settings"
            onClick={() => setSettingsOpen(true)}
          >
            <SlidersHorizontal size={15} /> Visibilidad y acceso
          </button>
          <span className="relative">
            <select
              ...
              className="appearance-none border-0 bg-transparent py-1 pl-0 pr-5 text-xs text-muted ring-0 focus:text-fg focus:ring-0"
            >
              <option value="public_org">Público en la organización</option>
              <option value="internal_area">Interno del área</option>
          ...
          {areas.map((area) => (
            <label key={area.key} className="flex items-center gap-1.5">
              <input
                type="checkbox"
                checked={visibleTo.includes(area.key)}
                onChange={(event) => {
                ...
                className="h-4 w-4 rounded-xs border-control bg-sunken text-accent"
              />
              {area.name}
            </label>
          ))}
          ...
          >
            ...
            <div className="mobile-sheet-head">
              <div>
                ...
                <h2>Visibilidad de la nota</h2>
              </div>
              <button
                className="icon-button"
                aria-label="Cerrar visibilidad"
                onClick={() => setSettingsOpen(false)}
              >
                <X size={18} />
              </button>
            ...
            <div className="space-y-5 px-5 pb-6">
              ...
              <fieldset>
                <legend className="field-label">Visible en áreas</legend>
                <div className="space-y-2">
                  {areas.map((area) => (
                    <label key={area.key} className="mobile-check-row">
                      <span
      ...
      {historyOpen && (
        <HistoryDialog
          note={{ ...note, versions: historyVersions ?? note.versions }}
          local={{ title, body }}
...
function HistoryDialog({
  ...
  const previous =
    note.versions.find((version) => version.version < note.version) ??
    note.versions[0];
  return (
    <div className="modal-backdrop">
      <div className="modal-card max-w-5xl">
        ...
        <div className="compare-grid">
          <ComparePane
            label={`Anterior · v${previous?.version ?? note.version}`}
            meta={
              previous
                ? `${previous.editedBy} · ${new Date(previous.editedAt).toLocaleString("es-PE")}`
                : "No hay una versión anterior"
            }
            body={previous?.body ?? "Esta es la primera versión de la nota."}
          />
          <ComparePane
            label={`Actual · v${note.version}`}
            meta="Versión local"
            body={local.body}
          />
        </div>
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-card max-w-5xl">
        <div className="modal-head">
          <div>
            <p className="eyebrow text-warn">Conflicto de edición</p>
            <h2>La nota cambió mientras editabas</h2>
            <p>
              Compara ambas versiones antes de decidir. Nada se sobrescribirá
              automáticamente.
            </p>
          </div>
        </div>
        <div className="compare-grid">
          ...
          />
          <ComparePane
            label={`Remota · v${remote.version}`}
            meta={`${remote.updatedBy} · ${new Date(remote.updatedAt).toLocaleString("es-PE")}`}
            body={remote.body}
          />
        </div>
        <div className="flex justify-end gap-3 border-t border-line p-4">
          <button className="secondary-button" onClick={onKeepRemote}>
            Usar versión remota
          </button>
...
function ComparePane({
  ...
  return (
    <section className="min-w-0">
      ...
      <pre className="max-h-[55vh] overflow-auto whitespace-pre-wrap p-5 font-mono text-xs leading-6 text-fg-2">
        {body}
      </pre>
    </section>
  );
