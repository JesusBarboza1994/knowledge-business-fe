...
import { BookOpen, ChevronDown, CirclePlus, File, FileClock, FileText, FolderKanban, GitGraph, LayoutPanelLeft, LogOut, Menu, Network, PanelRightClose, PanelRightOpen, Search, Settings2, Shield, Trash2, TriangleAlert, X } from 'lucide-react'
...
export function Workspace({ session, initialAreas, initialNotes, initialMembers, onLogout }: Props) {
  ...
  return <div className="flex h-screen overflow-hidden bg-ink text-stone-200">
    ...
    <main className="flex min-w-0 flex-1 flex-col">
      <div className="flex min-h-10 items-center border-b border-line bg-[#11130f]">
        <button className="mx-2 text-muted lg:hidden" onClick={() => setMobileNav(true)}><Menu size={18} /></button>
        ...
        <button className="icon-button mx-2" onClick={() => setRightOpen((value) => !value)} title="Alternar panel de contexto">{rightOpen ? <PanelRightClose size={16} /> : <PanelRightOpen size={16} />}</button>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        {rightOpen && view !== 'admin' && <Inspector note={selectedNote} notes={notes} areas={areas} onOpenNote={openNote} onGraph={() => setView('graph')} onDelete={() => setDeleteNoteOpen(true)} canDelete={area?.access === 'manage'} />}
      </div>
    </main>
